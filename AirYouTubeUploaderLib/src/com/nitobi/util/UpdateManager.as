package com.nitobi.util
{
	...
	public class UpdateManager extends EventDispatcher
	{
		...
		// retrieved from application descriptor, used for comparison with latestVersion
		...
		private var _downloadedFilePath:String = "temp.air";
		...
		[Event(name=UPDATE_AVAILABLE,type="Event")]
		
		public static const UPDATE_READY:String = "update_ready";
		
		[Event(name=UPDATE_READY,type="Event")]
		
		/* This is a sample of the xml for version listings ( note this is a very simple implementation  - jm)
		
			<?xml version="1.0" encoding="utf-8"?>
			<versions>
				<version path="http://someserver/somepath/somefile.air">0.0.1945</version>
			</versions>
		*/
		...
		private function handleXmlLoadComplete(evt:Event):void
		{
			...
			if(isUpdateAvailable)
			{
				...
				dispatchEvent(new Event(UPDATE_AVAILABLE));
			}
		}
		...
		*/
		public function downloadUpdate():void
		{
			...
			_urlStream.addEventListener(IOErrorEvent.IO_ERROR,onDownloadUpdateError);
			_urlStream.load(new URLRequest(_updateInstallUrl));
		}
		
		protected function onDownloadUpdateError(err:IOErrorEvent):void
		{
			trace("Error downloading update: " + err.text);
		}
		...
		{
		    this.dispatchEvent(new Event(UPDATE_READY));
		}
